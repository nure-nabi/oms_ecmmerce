...
class TopCategoryProduct extends StatefulWidget {
  int brandId;
  TopCategoryProduct({super.key,required this.brandId});

  @override
  State<TopCategoryProduct> createState() => _TopCategoryProductState();
}

class _TopCategoryProductState extends State<TopCategoryProduct> {
  final ScrollController _scrollController = ScrollController();
  @override
  void initState() {
    super.initState();
    // Initial load
    context.read<BrandWiseProductsBloc>().add(BrandWiseProductsReqEvent(limit: 20, offset: 0, brandId: widget.brandId));

    _scrollController.addListener(() {
      if (_scrollController.position.pixels >=
          _scrollController.position.maxScrollExtent - 200) {
        // Load more when 200px from bottom
        context.read<BrandWiseProductsBloc>().add(BrandWiseProductsLazyLoadEvent(limit: 20, offset: 0, brandId: widget.brandId));
      }
    });
  }
    ...
    return Scaffold(
      ...
      body: BlocConsumer<BrandWiseProductsBloc,BrandWiseProductsState>(
        ...
        listener: (context, state) {
          if (state is BrandWiseProductsErrorState) {
