...
class AddressUpdatePage extends StatefulWidget {
  ...
  final int index;
  const AddressUpdatePage({super.key,this.addressList,required this.addressUpdate,required this.index});
...
class _AddressUpdatePageState extends State<AddressUpdatePage> {
  ...
  int? provienceTempIndexMap = 0;
  int? cityTempIndexMap = 0;
  int? zoneTempIndexMap = 0;
  int? provienceId;
  int? cityId;
  int? zoneId;
     ...
     return SafeArea(
        child: Scaffold(
          appBar: AppBar(
            ...
            leading: InkWell(
                onTap: (){
                  Navigator.pushReplacementNamed(context, addressShow);
              ...
              child: Column(
                children: [
                  ...
                  BlocBuilder<ProvienceBloc,ProvienceState>(builder: (BuildContext context, state) {
                    if(state is ProvienceLoadingState){
                      return Text("Loading.....");
                    }
                    else if(state is ProvienceLoadedState){
                      ...
                      int indexProvience =0;
                      int indexCity =0;
                      int indexZone =0;

                      bool hasValidCityIndex = false;
                      bool hasCities = false;
                      bool hasZones = false;
                      if(state.indexProvience! >= 0){
                        ...
                        hasValidCityIndex = state.indexProvience != null &&
                            state.indexProvience! >= 0 &&
                            state.indexProvience! < state.provienceRes!.data.length;

                        // Check if cityies exists and is not empty (if indexCity is valid)
                        hasCities = hasValidCityIndex &&
                            state.provienceRes!.data[state.indexProvience!].cityies!.isNotEmpty;
                        ...
                        final hasValidZoneIndex = hasCities
                            ...
                            state.indexZone! >= 0 &&
                            state.indexZone! < state.provienceRes!.data[state.indexProvience!].cityies!.length;

                        // Check if zone exists and is not empty (if hasValidZoneIndex is true)
                        hasZones = hasValidZoneIndex &&
                            state.provienceRes!.data[state.indexProvience!].cityies![state.indexCity!].zone.isNotEmpty;
                      }else{
                        // Check if indexCity is valid

                        indexProvience = state.provienceRes!.data.indexWhere((value)=> value.name == widget.addressList![widget.index].provience!.province_name);
                        indexCity = state.provienceRes!.data[indexProvience].cityies!.indexWhere((value)=> value.name == widget.addressList![widget.index].city!.city);
                        indexZone = state.provienceRes!.data[indexProvience].cityies![indexCity].zone.indexWhere((value)=> value.name == widget.addressList![widget.index].zone!.zone_name);

                        if(provienceTempIndexMap == 0){
                          provienceId = state.provienceRes!.data[indexProvience].id!;
                        }

                        if(cityTempIndexMap == 0){
                          cityId = state.provienceRes!.data[indexProvience].cityies![indexCity].id!;
                        }

                        if(zoneTempIndexMap == 0){
                          zoneId = state.provienceRes!.data[indexProvience].cityies![indexCity].zone[indexZone].id!;
                        }



                        final effectiveCityIndex = indexCity;
                        hasValidCityIndex = effectiveCityIndex != null &&
                            effectiveCityIndex >= 0 &&
                            effectiveCityIndex < state.provienceRes!.data.length;

                        // Check if cityies exists and is not empty (if indexCity is valid)
                        hasCities = hasValidCityIndex &&
                            state.provienceRes!.data[indexProvience].cityies!.isNotEmpty;

                        // Check if indexZone is valid (if hasCities is true)
                        final effectiveZoneIndex = indexZone ;
                        // final effectiveZoneIndex = state.indexZone == 0 ? indexZone : state.indexZone;
                        final hasValidZoneIndex = hasCities &&
                            effectiveZoneIndex != null &&
                            effectiveZoneIndex >= 0 &&
                            effectiveZoneIndex < state.provienceRes!.data[indexProvience ].cityies!.length;


                        hasZones = hasValidZoneIndex &&
                            state.provienceRes!.data[indexProvience].cityies![indexCity].zone.isNotEmpty;
                                ...
                                child: DropdownButton2<String>(
                                        ...
                                        child: Padding(
                                          ...
                                          child: Container(
                                            ...
                                            child: Column(
                                              ...
                                              children: [
                                                Text(
                                                  party.name.toString(),style: GoogleFonts.poppins(
                                                    color: Colors.black
                                                ),
                                  ...
                                  selectedItemBuilder: (BuildContext context) {
                                    return state.provienceRes!.data.map((party) {
                                      return Text(
                                        party.name!,maxLines: 1,overflow: TextOverflow.ellipsis,
                              ...
                              Container(
                                ...
                                child: DropdownButtonHideUnderline(
                                  child: DropdownButton2<String>(
                                    ...
                                    items: state.provienceRes!.data[state.indexProvience! == -1 ? indexProvience : state.indexProvience!].cityies!.map((party) =>
                                        DropdownMenuItem<String>(
                                          value: party.name.toString(),
                                          child: Padding(
                                            padding: const EdgeInsets.only(bottom: 8),
                                            child: Container(
                                              ...
                                              child: Column(
                                                ...
                                                children: [
                                                  Text(
                                                    party.name.toString(),maxLines: 1,overflow: TextOverflow.ellipsis,style: GoogleFonts.poppins(
                                                      color: Colors.black
                                                  ),
                                    ...
                                    // Custom display of the selected item
                                    selectedItemBuilder: (BuildContext context) {
                                      return state.provienceRes!.data[state.indexProvience! == -1 ? indexProvience : state.indexProvience!].cityies!.map((party) {
                                        return Text(
                                          party.name!,
                                    ...
                                    onChanged: (value) {
                                      ...
                                      if(index != -1){
                                        context.read<ProvienceBloc>().add(ProvienceZoneIndexEvent(
                                          ...
                                          province: state.provienceValue,
                                          city: value,));
                                        cityId = state.provienceRes!.data[state.indexProvience! == -1 ? indexProvience : state.indexProvience!].cityies![index].id!;
                                        cityTempIndexMap = 1;
                                      }
                                    },
                                    buttonStyleData: ButtonStyleData(
                              ...
                              Container(
                                ...
                                child: DropdownButtonHideUnderline(
                                  child: DropdownButton2<String>(
                                    ...
                                    items: state.provienceRes!.data[state.indexProvience! == -1 ? indexProvience : state.indexProvience!].cityies![state.indexCity == 0 ? indexCity : state.indexCity!].zone.map((party) =>
                                        DropdownMenuItem<String>(
                                          value: party.name.toString(),
                                          child: Padding(
                                            padding: const EdgeInsets.only(bottom: 8),
                                    ...
                                    // Custom display of the selected item
                                    selectedItemBuilder: (BuildContext context) {
                                      return state.provienceRes!.data[state.indexProvience! == -1 ? indexProvience : state.indexProvience!].cityies![state.indexCity == 0 ?indexCity : state.indexCity!].zone.map((party) {
                                        return Text(
                                          party.name!,
                                    ...
                                    onChanged: (value) {
                                      int index = state.provienceRes!.data[state.indexProvience! == -1 ? indexProvience : state.indexProvience!].cityies![state.indexCity == 0 ?indexCity : state.indexCity!].zone.indexWhere(
                      ...
                      return Container(child: Text("Loaidng...."),);
                    }
                  },),
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: 10,),
                        if(widget.addressUpdate == "Update")...[
                          BlocBuilder<AddressBloc,AddressState>(builder: (BuildContext context, state) {
                            if(state is AddressLoadedState) {
                              return  Row(
                                children: [
                                  InkWell(
                                    onTap: () {
                                      context.read<AddressBloc>().add(AddressReqEvent(typeHomeFlag: true,typeOfficeFlag: false,shippingFlag: state.shippingFlag,billingFlag: state.billingFlag));
                                    },
                                    child: Container(
                                      height: 50,
                                      width: 50,
                                      decoration: BoxDecoration(
                                          color: state.addressTypeHomeFlag == true
                                              ? gPrimaryColor
                                              : Colors.white,
                                  ...
                                  InkWell(
                                    ...
                                    child: Container(
                                      ...
                                      width: 50,
                                      decoration: BoxDecoration(
                                          color: state.addressTypeOfficeFlag ==
                                              true ? gPrimaryColor : Colors.white,
                                          borderRadius: BorderRadius.all(
                                              Radius.circular(10)),
                                          border: Border.all(
                                              width: 1, color: Colors.grey)
                                      ),
                                      child: Icon(Bootstrap.building,
                                        color: state.addressTypeOfficeFlag == true
                                            ? Colors.white
                                            : Colors.black,),
                                    ),
                                  )
          ...
          bottomNavigationBar: Container(
            height: 65,
            child: BlocBuilder<AddressBloc,AddressState>(builder: (BuildContext context, state) {
                ...
                return  Padding(
                  ...
                  child: InkWell(
                    ...
                    child: Container(
                      ...
                      child: Center(
                        child: Text("Update Address",style: GoogleFonts.poppins(
                            color: Colors.white
                        ),),
                      ),
                    ),
                  ...
                  child: InkWell(
                    onTap: (){
                      ...
                      AddressModel addressModel = AddressModel(
                          id: widget.addressList![widget.index].id!.toString(),
                    ...
                    child: Container(
                      ...
                      child: Center(
                        child: Text(widget.addressUpdate == "Update" ? "Update Address" : "Save Address",style: GoogleFonts.poppins(
                            color: Colors.white
                        ),),
                      ),
                    ),
