...
class DeliveredOrder extends StatefulWidget {
  String status;
  DeliveredOrder({super.key, required this.status});
...
class _DeliveredOrderState extends State<DeliveredOrder> {
    ...
    return RefreshIndicator(
      ...
      child: BlocBuilder<OrderBloc,OrderState>(builder: (BuildContext context, state) {
        if(state is OrderLoadingState){
          ...
          // List<Widget> widgets = [];
          // for(int i = 0; i< state.orderResponse!.orders.orders.length; i++){
          //
          //   for(int j = 0; j<state.orderResponse!.orders.orders[i].orderItems.length; j++){
          //   //  return Text(state.orderResponse!.orders.orders[i].orderItems[j].order_id!.toString());
          //     widgets.add(_buildOrderItems(
          //         index: j,
          //         orderModel: state.orderResponse!.orders.orders[i],
          //         orderItemModel: state.orderResponse!.orders.orders[i].orderItems[j],
          //         orderList: state.orderResponse!.orders.orders
          //     ));
          //   }
          // }
          //
        ...
        // return Text("");
          ...
          return Padding(
            ...
            ) : Center(child: Image.asset("assets/icons/delivered.webp",width: 150,),),
          );
        }else{
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          //
          //           if(orderModel.orderItems[0].reviewed == 0)
          //           InkWell(
          //             onTap: (){
          //               Navigator.pushNamed(context, productReviewAndRatingPage,
          //               arguments: {
          //                 'productsModel' : orderModel.orderItems[0].productsModel,
          //                 'orderId':orderModel.order_id.toString()
          //               }
          //               );
          //             },
          //             child: Text("Write review",style: GoogleFonts.poppins(
          //               color: gPrimaryColor
          //             ),),
          //           )
          //
          ...
          //   ],
          // ),
        //  for(int i = 1; i< orderModel.orderItems.length ; i++)
           // Text(orderModel.orderItems.length.toString())

         // Text(orderModel!.orderItems.length.toString()),
           Column(
             children:
               List.generate(orderModel!.orderItems.length, (index1){
                 var orderItem = orderModel.orderItems[index1];
  ...
  Widget _buildOrderItems({
    ...
    return InkWell(
      onTap: (){
       // Fluttertoast.showToast(msg: indexOrder.toString());
          ...
          children: [
            Container(
              ...
              child: ClipRRect(
                borderRadius: BorderRadius.circular(3),
                child: CachedNetworkImage(
                  imageUrl:  orderItemModel!.productsModel!.main_image_full_url! != "" ?orderItemModel!.productsModel!.main_image_full_url! : orderItemModel!.productsModel!.image_full_url!,
                  placeholder: (context, url) =>Shimmer.fromColors(
                    baseColor: Colors.grey.shade300,
                    highlightColor: Colors.grey.shade100,
                    child:   SizedBox(
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 4),
                  Text(
                    orderItemModel!.productsModel!.product_name!,
                    style: TextStyle(
                      color: Colors.grey.shade600,
                      fontSize: 16,
                    ),
